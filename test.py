...
def test_one_to_one(task_load, task_eval, model, score_dict):
    ...
    cnt = 0
    for n_steps, (cqs, len_cqs, _, _, _, _, _, _, _) in enumerate(test_dataloader):
        # assume n_gpus == 1
        cqs = cqs[0]
        len_cqs = len_cqs[0]
                ...
                if len(qa_results[cnt]) not in [max_tot_lens[cnt], args.max_len]:
                    ...
                    if args.model_name == "gpt2":
                        for layer_id in range(MODEL_CONFIG.n_layer):
                            ...
                            all_pasts[layer_id][cnt] = pasts[layer_id][:, i, ..., :len_cqs[i], :].type(torch.float32 if args.fp32 else torch.half).cpu()
            cnt += 1

        if n_steps % 20 == 0:  # dynamic threshold to avoid out of memory
...
if __name__ == '__main__':
    ...
    # add task gen token before all to help with type error
