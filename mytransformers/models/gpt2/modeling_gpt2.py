...
class Block(GPT2DecoderBlockAdaptersMixin, nn.Module):
    ...
    def enable_adapter_transfer(self):
        adapter_names = [self.adapter_function[-1]]
        p_adapter_names = parse_composition(adapter_names)
        self.attention_adapters.enable_adapters(p_adapter_names, True, False)
        self.output_adapters.enable_adapters(p_adapter_names, True, False)
        return adapter_names

    """
    def load_old_adapter(self, src, tgt):
        state_dict = src.state_dict()
        new_state_dict = OrderedDict()
        for i in state_dict:
            new_state_dict[i] = state_dict[i].clone().detach()
        tgt.load_state_dict(new_state_dict)
    """

    def load_attention_weight(self, src, tgt):
        state_dict = self.attention_adapters.adapters[src].state_dict()
        new_state_dict = OrderedDict()
        for i in state_dict:
            new_state_dict[i] = state_dict[i].clone().detach()
        self.attention_adapters.adapters[tgt].load_state_dict(new_state_dict)

    def load_output_weight(self, src, tgt):
        state_dict = self.output_adapters.adapters[src].state_dict()
        new_state_dict = OrderedDict()
        for i in state_dict:
            new_state_dict[i] = state_dict[i].clone().detach()
        self.output_adapters.adapters[tgt].load_state_dict(new_state_dict)
    ...
    def setup_task_adapter(self, cnt, tid):
        ...
        self.ita = None
        del_list = []
        for name in self.attention_adapters.adapters:
            if name not in self.adapter_function:
                del_list.append(name)
        for name in del_list:
            del self.attention_adapters.adapters[name]
            del self.output_adapters.adapters[name]
    ...
    def forward(
        ...
        if self.uni_adapter is None:
            self.uni_adapter = np.unique(self.adapter_function)
        hidden_states = self.attention_adapters.mix_adapters_forward(attn_output, hidden_states, self.ita, 
        ...
        hidden_states = self.output_adapters.mix_adapters_forward(feed_forward_hidden_states, hidden_states, self.ita, 
                                                                     self.config.batch_task_id, self.config.forward_mode, 
                                                                     self.uni_adapter, self.adapter_function)
