...
class FP16_Module(nn.Module):
    ...
    def get_prompt(self, bsz):
        return self.module.get_prompt(bsz)

    def get_gpt(self):
        return self.module.get_gpt()

    def get_pasts(self):
        return self.module.get_pasts()

    def freeze(self):
        return self.module.freeze()
...
class FP16_Optimizer(object):
    ...
    **Multi_GPU training**:  If the wrapped ``init_optimizer`` was created from a model wrapped in
    ...
    def step(self, **kwargs): # could add clip option.
        """
        ...
        """
        # Disable closure by ZYZ 20210517 to pass reg_params
        closure = None
        scale = self.loss_scaler.loss_scale
        self._update_scale(self.overflow)

        if self.overflow:
            # Note that there are probably several steps of overflow at the beginning of training...
            # logger.info("One step is overflow?!....")
            self.maybe_print("OVERFLOW! Skipping step. Attempted loss scale: {}, reducing to {}".format(scale, self.loss_scale))
            return

        if closure is not None:
            # logger.info("One step with closure...")
            retval = self._step_with_closure(closure)
        else:
            # logger.info("One step without closure...")
            retval = self.optimizer.step(**kwargs)
