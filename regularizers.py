    ...
    def step(self, reg_params, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # moved by ZYZ due to explosion/nan
                """
                if p in reg_params:
...
def compute_importance(model, parallel_model, updater, dataloaders, loss_type="l2"):
    ...
    if loss_type == "l2":
        ...
        while model.vocab_size != TOKENS_WEIGHT.shape[0]:
            TOKENS_WEIGHT = torch.cat((TOKENS_WEIGHT, torch.ones([1]).cuda()))
        CELoss = CrossEntropyLoss(ignore_index=FILL_VAL, reduction='mean', weight=TOKENS_WEIGHT)
        # loss_fct = DataParallelCriterion(CELoss, args.device_ids)
        loss_fct = CELoss

    # Iterate over data.
    for dataloader in dataloaders:
        for cq, len_cq, cqa, len_cqa, Y, _, _, _, _ in dataloader:
            # get the inputs
            ...
            if loss_type != "ewc":
                ...
                targets, _ = get_losses(parallel_model, cqa[0].cuda(), Y[0].cuda(), None, None, loss_fct)
